 ...
 * (nightly/on-demand via odata_get_all) naar Mímir i.p.v. BC.
 *
 * Met $mimirApi gezet zijn $auth_list / $environment / $baseUrl / $auth ongebruikt voor BC;
 * Mímir beheert environments — FinRap heeft alleen de API-key (+ optioneel $mimirBase) nodig.
 *
...
/**
 * Placeholder-host zodat bestaande URL-builders een pad maken dat
 * odata_mimir_parse_entity_url kan lezen. Er gaat geen request naartoe.
 */
function odata_mimir_placeholder_base_url(): string
{
    return 'https://mimir.local';
}
