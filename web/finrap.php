...
<div class="project-overlay">
    <div class="project-modal" role="dialog" aria-modal="true" aria-labelledby="projectModalTitle">
        <div class="project-modal-head">
            <strong class="project-modal-title" id="projectModalTitle">Financieel Rapport <?= htmlspecialchars($reportProjectNo) ?></strong>
            <div class="project-modal-head-actions">
                <button class="project-close" type="button" onclick="window.location.href='index.php?company=<?= rawurlencode($company) ?>'">Terug</button>
                <button class="project-close" type="button" onclick="window.print()">Print</button>
            </div>
        </div>
    ...
    <?php else: ?>
            ...
            <section class="project-modal-info-panel">
                ...
                <div class="project-modal-column">
                    <div class="project-field">
                        ...
                        <div class="project-field-value"><?= htmlspecialchars((string) ($project['Creation_Date'] ?? '')) ?></div>
                    </div>
                    <div class="project-field">
                        <div class="project-field-label">Completed date</div>
                        <div class="project-field-value"><?= htmlspecialchars((string) ($project['Ending_Date'] ?? '')) ?></div>
                    </div>
                    <div class="project-field">
                        <div class="project-field-label">Sales Manager</div>
                        <div class="project-field-value"><?= htmlspecialchars((string) ($project['KVT_Sales_Person_Code'] ?? '')) ?></div>
                    </div>
                </div>
            </section>
            ...
            <section class="project-modal-summary-section">
                <table class="project-metric-table">
                    ...
                    <tbody>
                    <tr>
                        <td class="is-right"><?= htmlspecialchars(finrap_format_currency($contractValue)) ?></td>
                        <td class="is-right"><?= htmlspecialchars(finrap_format_currency($totalDirectCost)) ?></td>
                        <td class="is-right <?= $grossProfit >= 0 ? 'is-positive' : 'is-negative' ?>"><?= htmlspecialchars(finrap_format_currency($grossProfit)) ?></td>
                        <td class="is-right <?= $variance >= 0 ? 'is-positive' : 'is-negative' ?>"><?= htmlspecialchars(finrap_format_currency($variance)) ?></td>
                        <td class="is-right <?= $orderResult >= 0 ? 'is-positive' : 'is-negative' ?>"><?= htmlspecialchars(finrap_format_currency($orderResult)) ?></td>
                        <td class="is-right"><?= htmlspecialchars(finrap_format_currency($installmentsInvoiced)) ?></td>
                        <td class="is-right"><?= htmlspecialchars(finrap_format_currency($installmentsReceived)) ?></td>
                    </tr>
                    </tbody>
                </table>
            </section>
                ...
                <table class="project-cost-group-table">
                    ...
                    <?php foreach ($taskRows as $row): ?>
                        ...
                        <tr class="<?= $rowClass ?>">
                            ...
                            <td class="is-right"><?= htmlspecialchars(finrap_format_currency((float) ($row['Budget_Cost'] ?? 0.0))) ?></td>
                            <td class="is-right"><?= htmlspecialchars(finrap_format_currency((float) ($row['EAC'] ?? 0.0))) ?></td>
                            <td class="is-right"><?= htmlspecialchars(finrap_format_currency((float) ($row['Booked_Cost'] ?? 0.0))) ?></td>
                            <td class="is-right"><?= htmlspecialchars(finrap_format_currency((float) ($row['Entered_Obligations'] ?? 0.0))) ?></td>
                            <td class="is-right <?= $varianceBudget >= 0 ? 'is-positive' : 'is-negative' ?>"><?= htmlspecialchars(finrap_format_currency($varianceBudget)) ?></td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if ($taskTotal !== []): ?>
                        <tr class="is-grand-total-row">
                            <td><?= htmlspecialchars((string) ($taskTotal['Cost_Group_Code'] ?? 'TOTAL')) ?></td>
                            <td class="is-description"><?= htmlspecialchars((string) ($taskTotal['Cost_Group_Description'] ?? 'Totaal alle regels')) ?></td>
                            <td class="is-right"><?= htmlspecialchars(finrap_format_currency((float) ($taskTotal['Budget_Cost'] ?? 0.0))) ?></td>
                            <td class="is-right"><?= htmlspecialchars(finrap_format_currency((float) ($taskTotal['EAC'] ?? 0.0))) ?></td>
                            <td class="is-right"><?= htmlspecialchars(finrap_format_currency((float) ($taskTotal['Booked_Cost'] ?? 0.0))) ?></td>
                            <td class="is-right"><?= htmlspecialchars(finrap_format_currency((float) ($taskTotal['Entered_Obligations'] ?? 0.0))) ?></td>
                            <?php $totalVariance = (float) ($taskTotal['Variance_Budget_EAC'] ?? 0.0); ?>
                            <td class="is-right <?= $totalVariance >= 0 ? 'is-positive' : 'is-negative' ?>"><?= htmlspecialchars(finrap_format_currency($totalVariance)) ?></td>
                        </tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            ...
            <section class="project-modal-summary-section" style="padding-top:0;">
                <p class="project-modal-subtitle" style="margin:0;">
                    Bedrijf: <?= htmlspecialchars($company) ?> | Maand: <?= htmlspecialchars(finrap_month_label($yearMonth)) ?>
                    <?php if ($createdAt !== ''): ?> | Opgehaald: <?= htmlspecialchars($createdAt) ?><?php endif; ?>
                </p>
            </section>
    <?php endif; ?>
