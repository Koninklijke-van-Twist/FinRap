...
const FINRAP_PLANNING_LINES_ENTITY_SET = 'LVS_JobPlanningLinesSub';
const FINRAP_ITEM_CARD_ENTITY_SET = 'AppItemCard';
const FINRAP_STANDARD_COSTING_METHOD = 'Standard';
const FINRAP_ITEM_COSTING_METHOD_CHUNK_SIZE = 40;
const FINRAP_TO_BOOK_COST_FIELD = 'To_Book_Cost';
const FINRAP_COSTS_TOTAL_FIELD = 'Costs_Total';
const FINRAP_PURCHASE_LINE_ENTITY_SETS = [
...
function finrap_report_index_entry_from_payload(string $company, string $projectNo, string $reportId, array $payload): array
{
    ...
    $taskRows = finrap_apply_report_overrides_to_task_rows($taskRows, $overrides);
    $summaryTotals = finrap_get_report_summary_totals($taskRows);
    $bookedCost = finance_to_float($summaryTotals['Booked_Cost'] ?? 0.0);
    $toBookCost = finance_to_float($summaryTotals['To_Book_Cost'] ?? $summaryTotals['Unposted_Cost'] ?? 0.0);
    $pocCostProgress = finance_column_poc_cost_progress($bookedCost, $toBookCost);
    $budgetCost = finance_to_float($summaryTotals['Budget_Cost'] ?? 0.0);
    $eacTotal = finance_to_float($summaryTotals['EAC'] ?? 0.0);
    $bookedHours = finance_to_float($summaryTotals['Booked_Hours'] ?? 0.0);
    $budgetHours = finance_to_float($summaryTotals['Budget_Hours'] ?? 0.0);
    $eacHours = finance_to_float($summaryTotals['EAC_Hours'] ?? 0.0);
    ...
    return [
        ...
        'poc_baseline' => round(finrap_calculate_poc_percent($pocCostProgress, $budgetCost), 2),
        'poc_eac' => round(finrap_calculate_poc_percent($pocCostProgress, $eacTotal), 2),
        'poc_baseline_hours' => round(finrap_calculate_poc_percent($bookedHours, $budgetHours), 2),
        'poc_eac_hours' => round(finrap_calculate_poc_percent($bookedHours, $eacHours), 2),
    ];
}
...
function finrap_build_dashboard_poc_points_from_index(string $company, string $projectNo, ?array $index = null): array
{
    ...
    foreach ($selected as $entry) {
        ...
        if (
            is_array($indexEntry)
            && isset(
                $indexEntry['poc_baseline'],
                $indexEntry['poc_eac'],
                $indexEntry['poc_baseline_hours'],
                $indexEntry['poc_eac_hours']
            )
        ) {
            $points[] = [
                'date' => $chartDate,
                ...
                'poc_baseline_hours' => round(finance_to_float($indexEntry['poc_baseline_hours'] ?? 0.0), 2),
                'poc_eac_hours' => round(finance_to_float($indexEntry['poc_eac_hours'] ?? 0.0), 2),
            ];
            continue;
        }
        ...
        $points[] = [
            ...
            'poc_baseline_hours' => round(finance_to_float($metrics['poc_baseline_hours'] ?? 0.0), 2),
            'poc_eac_hours' => round(finance_to_float($metrics['poc_eac_hours'] ?? 0.0), 2),
        ];
    }
...
function finrap_build_project_dashboard_core(string $company, string $projectNo, ?array $index = null): array
{
    ...
    foreach ($points as $point) {
        ...
        $yMaxPercent = max(
            ...
            finance_to_float($point['poc_baseline_hours'] ?? 0.0),
            finance_to_float($point['poc_eac_hours'] ?? 0.0)
        );
    }
...
function finrap_reconcile_report_index(string $company, string $projectNo): bool
{
    ...
    if (is_array($index['reports'] ?? null)) {
        foreach ($index['reports'] as $entry) {
            if (
                !is_array($entry)
                || !array_key_exists('poc_baseline', $entry)
                || !array_key_exists('poc_baseline_hours', $entry)
                || !array_key_exists('poc_eac_hours', $entry)
            ) {
...
function finrap_compute_report_poc_metrics(string $company, string $projectNo, string $reportId): ?array
{
    ...
    return [
        'poc_baseline' => finrap_calculate_poc_percent($pocCostProgress, $budgetCost),
        'poc_eac' => finrap_calculate_poc_percent($pocCostProgress, $eacTotal),
        'poc_baseline_hours' => finrap_calculate_poc_percent($bookedHours, $budgetHours),
        'poc_eac_hours' => finrap_calculate_poc_percent($bookedHours, $eacHours),
    ];
}
...
function finrap_task_metrics_from_total_row(?array $totalRow): ?array
{
    ...
    return finrap_with_derived_cost_totals([
        'Budget_Revenue' => finance_to_float($totalRow['Budget_Revenue'] ?? 0.0),
        'Budget_Cost' => $budgetCost,
        'EAC' => $eac,
        'Booked_Cost' => finance_to_float($totalRow['Booked_Cost'] ?? 0.0),
        'Unposted_Cost' => finance_to_float($totalRow['Unposted_Cost'] ?? 0.0),
        'To_Book_Cost' => finance_to_float($totalRow['To_Book_Cost'] ?? $totalRow['Unposted_Cost'] ?? 0.0),
...
function finrap_is_standard_costing_method(string $costingMethod): bool
{
    return strcasecmp(trim($costingMethod), FINRAP_STANDARD_COSTING_METHOD) === 0;
}

function finrap_fetch_standard_costing_item_lookup(
    string $baseUrl,
    string $environment,
    string $company,
    array $auth,
    ...
    foreach ($itemNos as $itemNo) {
        $itemNoText = trim((string) $itemNo);
        if ($itemNoText === '') {
            continue;
        }
        $uniqueItemNos[strtolower($itemNoText)] = $itemNoText;
    }
    ...
    $standardLookup = [];
    $chunks = array_chunk(array_values($uniqueItemNos), max(1, FINRAP_ITEM_COSTING_METHOD_CHUNK_SIZE));
    foreach ($chunks as $chunk) {
        $filters = [];
        foreach ($chunk as $itemNoText) {
            $filters[] = "No eq '" . str_replace("'", "''", $itemNoText) . "'";
        }
        if ($filters === []) {
            continue;
        }
        ...
        try {
            $itemUrl = finrap_company_entity_url_with_query($baseUrl, $environment, $company, FINRAP_ITEM_CARD_ENTITY_SET, [
                '$select' => 'No,Costing_Method',
                '$filter' => '(' . implode(' or ', $filters) . ')',
            ]);
            $itemRows = odata_get_all($itemUrl, $auth, $ttl);
        } catch (Throwable $ignoredItemLoadError) {
            continue;
        }
        ...
        foreach ($itemRows as $itemRow) {
            ...
            $itemNoText = trim((string) ($itemRow['No'] ?? ''));
            if ($itemNoText === '') {
                continue;
            }

            if (!finrap_is_standard_costing_method((string) ($itemRow['Costing_Method'] ?? ''))) {
                continue;
            }

            $standardLookup[strtolower($itemNoText)] = true;
        }
    }
...
function finrap_parse_unposted_costs_by_task(array $planningRows, array $standardItemLookup): array
{
    $unpostedByTask = [];
    ...
    foreach ($planningRows as $planningRow) {
        ...
        $taskNo = trim((string) ($planningRow['Job_Task_No'] ?? ''));
        $itemNo = trim((string) ($planningRow['No'] ?? ''));
        if ($taskNo === '' || $itemNo === '') {
            continue;
        }

        if (!isset($standardItemLookup[strtolower($itemNo)])) {
            continue;
        }

        $qtyToTransfer = finance_to_float($planningRow['Qty_to_Transfer_to_Journal'] ?? 0.0);
        if (abs($qtyToTransfer) < 0.000001) {
            continue;
        }

        $lineUnposted = finance_column_unposted_cost_line(
            $qtyToTransfer,
            finance_to_float($planningRow['Unit_Cost_LCY'] ?? 0.0)
        );
        if (abs($lineUnposted) < 0.000001) {
            continue;
        }

        $taskKey = strtolower($taskNo);
        $unpostedByTask[$taskKey] = finance_add_amount(
            finance_to_float($unpostedByTask[$taskKey] ?? 0.0),
            $lineUnposted
        );
    }

    return $unpostedByTask;
}

function finrap_odata_get_all_with_select_fallback(
    string $baseUrl,
    string $environment,
    string $company,
    array $auth,
    int $ttl,
    string $entitySet,
    array $selectVariants,
    string $filter
): ?array {
    foreach ($selectVariants as $select) {
        $selectText = trim((string) $select);
        if ($selectText === '') {
            continue;
        }

        try {
            $url = finrap_company_entity_url_with_query($baseUrl, $environment, $company, $entitySet, [
                '$select' => $selectText,
                '$filter' => $filter,
            ]);
...
function finrap_fetch_purchase_lines_for_project(
    string $baseUrl,
    string $environment,
    string $company,
    array $auth,
    ...
    foreach (FINRAP_PURCHASE_LINE_ENTITY_SETS as $entitySet) {
        $entitySet = trim((string) $entitySet);
        if ($entitySet === '') {
            continue;
        }

        $rows = finrap_odata_get_all_with_select_fallback(
            $baseUrl,
            $environment,
            $company,
            $auth,
...
function finrap_row_completely_received_flag(array $row): mixed
{
    if (array_key_exists('LVS_Completely_Received', $row)) {
        return $row['LVS_Completely_Received'];
    }

    if (array_key_exists('Completely_Received', $row)) {
        return $row['Completely_Received'];
    }

    return false;
}

function finrap_row_purchase_received_amount(array $row): float
{
    $completelyReceived = finance_odata_flag_is_true(finrap_row_completely_received_flag($row));
    $lineAmount = $completelyReceived
        ? finance_to_float($row['Line_Amount'] ?? $row['Amount'] ?? 0.0)
        : 0.0;

    return finance_column_purchase_received_amount(
        finance_to_float($row['Amt_Rcd_Not_Invoiced'] ?? 0.0),
        finance_to_float($row['Quantity_Received'] ?? $row['Qty_Received'] ?? 0.0),
        finance_to_float($row['Direct_Unit_Cost'] ?? $row['Unit_Cost_LCY'] ?? 0.0),
        $lineAmount
    );
}
...
function finrap_parse_received_not_posted_by_task(
    ...
    foreach ($sourceRows as $row) {
        ...
        $lineAmount = finance_column_received_not_posted_line(
            ...
            $qtyPosted,
            finrap_row_purchase_received_amount($row)
        );
        if (abs($lineAmount) < 0.000001) {
            continue;
        }
        ...
        if ($itemTaskKey !== '') {
            $countedKeys['item:' . $itemTaskKey] = true;
        }

        $taskKey = strtolower($taskNo);
        $byTask[$taskKey] = finance_add_amount(
            finance_to_float($byTask[$taskKey] ?? 0.0),
            $lineAmount
        );
    }
...
function finrap_parse_qty_posted_by_item_task(array $planningRows): array
{
    $lookup = [];
    ...
    foreach ($planningRows as $planningRow) {
        ...
        $lookup[strtolower($taskNo) . '|' . strtolower($itemNo)] = finance_to_float($planningRow['Qty_Posted']);
    }
...
function finrap_parse_booked_purchase_costs_by_task(array $ledgerRows): array
{
    ...
    $byTask = [];
    foreach ($ledgerRows as $ledgerRow) {
        if (!is_array($ledgerRow) || !finance_ledger_type_is_purchase((string) ($ledgerRow['Type'] ?? ''))) {
            continue;
        }
...
function finrap_merge_amount_maps(array $left, array $right): array
{
    foreach ($right as $key => $amount) {
        $mapKey = strtolower(trim((string) $key));
        if ($mapKey === '') {
            continue;
        }

        $left[$mapKey] = finance_add_amount(
            finance_to_float($left[$mapKey] ?? 0.0),
            $amount
        );
    }
...
function finrap_with_derived_cost_totals(array $metrics): array
{
    $bookedCost = finance_to_float($metrics['Booked_Cost'] ?? 0.0);
    $toBookCost = finance_to_float($metrics['To_Book_Cost'] ?? $metrics['Unposted_Cost'] ?? 0.0);
    $metrics['To_Book_Cost'] = $toBookCost;
    $metrics['Costs_Total'] = finance_column_costs_total($bookedCost, $toBookCost);
...
function finrap_apply_to_book_costs_to_task_rows(
    ...
    foreach ($taskRowsByKey as $taskKey => &$taskRow) {
        ...
        $taskRow['Costs_Total'] = finance_column_costs_total(
            ...
            finance_to_float($taskRow['To_Book_Cost'])
        );
    }
    unset($taskRow);
}
...
function finrap_aggregate_detail_task_metrics_for_task_keys(array $detailTaskRows, array $allowedTaskKeys): array
{
    ...
    foreach ($detailTaskRows as $taskRow) {
        ...
        $unpostedTotal = finance_add_amount($unpostedTotal, finance_to_float($taskRow['Unposted_Cost'] ?? 0.0));
        $toBookTotal = finance_add_amount(
            $toBookTotal,
...
function finrap_build_single_header_metric_row(
    ...
    $hasPlanningContractTotals = array_key_exists('contract_by_group', $planningTotals)
        || array_key_exists('invoiced_by_group', $planningTotals);
    ...
    $row = [
        ...
        'to_book_cost' => finance_to_float($taskMetrics['To_Book_Cost'] ?? $taskMetrics['Unposted_Cost'] ?? 0.0),
        'costs_total' => finance_column_costs_total(
            finance_to_float($taskMetrics['Booked_Cost'] ?? 0.0),
            finance_to_float($taskMetrics['To_Book_Cost'] ?? $taskMetrics['Unposted_Cost'] ?? 0.0)
        ),
...
function finrap_collect_modal_data(string $company, string $projectNo, int $ttl): array
{
    ...
    $billablePlanningSelect = 'Job_No,Line_No,Line_Type,Job_Task_No,Type,No,Description,Description_2,Document_No,'
    ...
    try {
        ...
        try {
            ...
            $contractRows = [];
            $planningTotalsAvailable = false;
        }
    }

    $contractRows = $planningTotalsAvailable
        ? finrap_filter_gl_revenue_planning_rows($contractRows)
        : [];
    $planningTotals = $planningTotalsAvailable
        ? finrap_parse_gl_revenue_planning_totals($contractRows, $changeOrderByTask)
        : [];
    $contractGroups = $planningTotalsAvailable
        ? [
        ...
        : finrap_parse_project_task_contract_groups($projectTaskRows, $projectNo);
    ...
    $ledgerRows = finrap_odata_get_all_with_select_fallback(
        $baseUrl,
        $environment,
        $company,
        $auth,
    ...
    $planningRows = finrap_odata_get_all_with_select_fallback(
        $baseUrl,
        $environment,
        $company,
        $auth,
    ...
    foreach ($planningRows as $planningRow) {
        ...
        $candidateItemNos[$itemNo] = $itemNo;
    }

    $standardItemLookup = finrap_fetch_standard_costing_item_lookup(
        $baseUrl,
        $environment,
        $company,
        $auth,
        $ttl,
        array_values($candidateItemNos)
    );
    $unpostedCostsByTask = finrap_parse_unposted_costs_by_task($planningRows, $standardItemLookup);
    $qtyPostedByItemTask = finrap_parse_qty_posted_by_item_task($planningRows);
    $purchaseRows = finrap_fetch_purchase_lines_for_project(
        $baseUrl,
        $environment,
        $company,
        $auth,
    ...
    foreach ($planningRows as $planningRow) {
        ...
        // Qty_to_Transfer_to_Journal already sits in Unposted_Cost; do not also treat that line as goods-in.
        if (abs(finance_to_float($planningRow['Qty_to_Transfer_to_Journal'] ?? 0.0)) >= 0.000001) {
            continue;
        }

        $planningReceivedSourceRows[] = $planningRow;
    }
    ...
    $contractByTask = $planningTotalsAvailable
        ? (is_array($planningTotals['contract_by_task'] ?? null)
            ? $planningTotals['contract_by_task']
            : [])
        : finrap_parse_project_task_contract_by_task(
            $projectTaskRows,
            array_keys($taskRowsByKey),
            $projectNo
        );
    ...
    foreach ($hoursRows as $hoursRow) {
        ...
        $hoursUsedQuantity = finance_to_float($hoursRow['LVS_Used_Hours_Quantity'] ?? 0.0);
        $hoursBudgetFallback = finance_add_amount($hoursBudgetFallback, $hoursBudgetQuantity);
        $hoursBooked = finance_add_amount($hoursBooked, $hoursUsedQuantity);
        if ($hoursTaskKey !== '' && isset($bookingRows[$hoursTaskKey])) {
            ...
            $hoursBookedByTask[$hoursTaskKey] = finance_add_amount(
                finance_to_float($hoursBookedByTask[$hoursTaskKey] ?? 0.0),
                $hoursUsedQuantity
            );
        }
    ...
    foreach ($bookingRows as $taskKey => $bookingRow) {
        ...
        $bookedHours = finance_to_float($hoursBookedByTask[$taskKey] ?? 0.0);
        $taskRowsByKey[$taskKey]['Budget_Hours'] = $budgetHours;
        $taskRowsByKey[$taskKey]['Booked_Hours'] = $bookedHours;
        ...
        if ((bool) ($taskRow['Is_Total_Row'] ?? false)) {
            ...
            if ($range !== null) {
                foreach ($bookingRows as $bookingRow) {
                    ...
                    $unpostedTotal = finance_add_amount($unpostedTotal, finance_to_float($bookingRow['Unposted_Cost'] ?? 0.0));
                    $toBookTotal = finance_add_amount(
                        $toBookTotal,
                        finance_to_float($bookingRow['To_Book_Cost'] ?? $bookingRow['Unposted_Cost'] ?? 0.0)
                    );
                ...
                $taskRowsByKey[$taskKey]['Unposted_Cost'] = $unpostedTotal;
                $taskRowsByKey[$taskKey]['To_Book_Cost'] = $toBookTotal;
                $taskRowsByKey[$taskKey]['Costs_Total'] = finance_column_costs_total($bookedTotal, $toBookTotal);
    ...
    $modal['task_rows_total'] = [
        ...
        'EAC_Hours' => $aggregatedTotals['EAC_Hours'],
        'Booked_Hours' => $aggregatedTotals['Booked_Hours'],
        'Booked_Cost' => $aggregatedTotals['Booked_Cost'],
        'Unposted_Cost' => $aggregatedTotals['Unposted_Cost'],
        'To_Book_Cost' => $aggregatedTotals['To_Book_Cost'],
        'Costs_Total' => $aggregatedTotals['Costs_Total'],
...
function finrap_aggregate_detail_task_rows(array $taskRows): array
{
    ...
    foreach ($taskRows as $taskRow) {
        ...
        $eacTotal = finance_add_amount($eacTotal, finance_to_float($taskRow['EAC'] ?? 0.0));
        $eacHoursTotal = finance_add_amount($eacHoursTotal, finance_to_float($taskRow['EAC_Hours'] ?? 0.0));
        $bookedHoursTotal = finance_add_amount($bookedHoursTotal, finance_to_float($taskRow['Booked_Hours'] ?? 0.0));
        $bookedTotal = finance_add_amount($bookedTotal, finance_to_float($taskRow['Booked_Cost'] ?? 0.0));
        $unpostedTotal = finance_add_amount($unpostedTotal, finance_to_float($taskRow['Unposted_Cost'] ?? 0.0));
        $toBookTotal = finance_add_amount(
            $toBookTotal,
...
function finrap_rollup_total_row_metrics(array &$taskRows): void
{
    foreach ($taskRows as &$taskRow) {
        ...
        foreach ($taskRows as $detailRow) {
            ...
            $eacHoursTotal = finance_add_amount($eacHoursTotal, finance_to_float($detailRow['EAC_Hours'] ?? 0.0));
            $bookedHoursTotal = finance_add_amount($bookedHoursTotal, finance_to_float($detailRow['Booked_Hours'] ?? 0.0));
            $bookedTotal = finance_add_amount($bookedTotal, finance_to_float($detailRow['Booked_Cost'] ?? 0.0));
            $unpostedTotal = finance_add_amount($unpostedTotal, finance_to_float($detailRow['Unposted_Cost'] ?? 0.0));
            $toBookTotal = finance_add_amount(
                $toBookTotal,
                finance_to_float($detailRow['To_Book_Cost'] ?? $detailRow['Unposted_Cost'] ?? 0.0)
            );
        ...
        $taskRow['EAC_Hours'] = $eacHoursTotal;
        $taskRow['Booked_Hours'] = $bookedHoursTotal;
        $taskRow['Booked_Cost'] = $bookedTotal;
        $taskRow['Unposted_Cost'] = $unpostedTotal;
        $taskRow['To_Book_Cost'] = $toBookTotal;
        $taskRow['Costs_Total'] = finance_column_costs_total($bookedTotal, $toBookTotal);
...
function finrap_normalize_loaded_task_row_fields(array $taskRows): array
{
    foreach ($taskRows as &$taskRow) {
        ...
        if (!array_key_exists('EAC_Hours', $taskRow)) {
            $taskRow['EAC_Hours'] = 0.0;
        }

        if (!array_key_exists('Booked_Hours', $taskRow)) {
            $taskRow['Booked_Hours'] = 0.0;
        }

        if (!array_key_exists('Unposted_Cost', $taskRow)) {
            $taskRow['Unposted_Cost'] = 0.0;
        }

        if (!array_key_exists('To_Book_Cost', $taskRow)) {
            $taskRow['To_Book_Cost'] = finance_to_float($taskRow['Unposted_Cost'] ?? 0.0);
        }

        $taskRow['Costs_Total'] = finance_column_costs_total(
            finance_to_float($taskRow['Booked_Cost'] ?? 0.0),
            finance_to_float($taskRow['To_Book_Cost'] ?? 0.0)
        );
...
function finrap_normalize_loaded_header_metric_rows(array $headerMetricRows): array
{
    foreach ($headerMetricRows as &$headerRow) {
        if (!is_array($headerRow)) {
            continue;
        }

        $unpostedCost = finance_to_float($headerRow['unposted_cost'] ?? 0.0);
        if (!array_key_exists('to_book_cost', $headerRow)) {
            $headerRow['to_book_cost'] = $unpostedCost;
        }
        $headerRow['costs_total'] = finance_column_costs_total(
            finance_to_float($headerRow['booked_cost'] ?? 0.0),
            finance_to_float($headerRow['to_book_cost'] ?? 0.0)
        );
    }
    unset($headerRow);

    return $headerMetricRows;
}

function finrap_normalize_override_map(array $overridesByTask): array
{
    $normalizedOverrides = [];
    foreach ($overridesByTask as $taskCode => $amount) {
